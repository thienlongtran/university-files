...
/* add pragma below to silence gcc warning about set but not used variable
 * on line 528 (prior to me adding these lines) -- m. toups */
...
static int test_0_arg(funct_t f, funct_t ft, char *name)
{
    ...
    if (error && !grade)
	printf(KRED "ERROR:" KNRM " Test " KYEL "%s()" KNRM " failed...\n...Gives %d[0x%x]. Should be %d[0x%x]\n", name, r, r, rt, rt);
...
static int test_1_arg(funct_t f, funct_t ft, int arg1, char *name)
{
    ...
    error = (r != rt);
    if (error && !grade)
	printf(KRED "ERROR:" KNRM " Test " KYEL "%s(%d[0x%x])" KNRM " failed...\n...Gives %d[0x%x]. Should be %d[0x%x]\n", name, arg1, arg1, r, r, rt, rt);
...
static int test_2_arg(funct_t f, funct_t ft, int arg1, int arg2, char *name)
{
    ...
    if (error && !grade)
	printf(KRED "ERROR:" KNRM " Test " KYEL "%s(%d[0x%x],%d[0x%x])" KNRM " failed...\n...Gives %d[0x%x]. Should be %d[0x%x]\n", name, arg1, arg1, arg2, arg2, r, r, rt, rt);
		      ...
		      int arg1, int arg2, int arg3, char *name)
{
    ...
    if (error && !grade)
	printf(KRED "ERROR:" KNRM " Test " KYEL "%s(%d[0x%x],%d[0x%x],%d[0x%x])" KNRM " failed...\n...Gives %d[0x%x]. Should be %d[0x%x]\n", name, arg1, arg1, arg2, arg2, arg3, arg3, r, r, rt, rt);
...
static int run_tests() 
{
    ...
    printf(KWHT "Score\tRating\tErrors\tFunction\n" KNRM);
